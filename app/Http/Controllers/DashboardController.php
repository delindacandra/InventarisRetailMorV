...
use App\Charts\SlowMovingChart;
use Illuminate\Support\Facades\DB;

class DashboardController extends Controller
{
    public function index(Top5BarangChart $top5chart, PermintaanBarangChart $permintaanBarangChart, TheMostFungsiChart $fungsiChart, SlowMovingChart $slowMovingChart)
    {
        $breadcrumb = (object)[
            'title' => 'Dashboard',
            'list' => ['Home', 'Dashboard']
        ...
        return view('dashboard', ['breadcrumb' => $breadcrumb, 'activeMenu' => $activeMenu, 'totalBarang'=>$totalBarang, 'totalBarangMasuk'=>$totalBarangMasuk, 'totalBarangKeluar'=>$totalBarangKeluar,
         'top5chart'=>$top5chart->build(), 'permintaanBarangChart'=>$permintaanBarangChart->build(), 'fungsiChart'=>$fungsiChart->build(), 'slowMovingChart'=>$slowMovingChart->build()]);
    }
}
